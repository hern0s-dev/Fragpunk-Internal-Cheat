...
kiero::Status::Enum kiero::init(RenderType::Enum _renderType)
{
	...
	if (_renderType != RenderType::None)
	{
		if (_renderType >= RenderType::D3D9 && _renderType <= RenderType::D3D12)
		{
			...
			HWND window = ::CreateWindow(windowClass.lpszClassName, KIERO_TEXT("Kiero DirectX Window"), WS_EX_COMPOSITED | WS_EX_LAYERED | WS_EX_TRANSPARENT | WS_EX_TOPMOST, 0, 0, 1920, 1080, NULL, NULL, windowClass.hInstance, NULL);
